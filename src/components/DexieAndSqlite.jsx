...
import {
  ...
  saveImageOffline,
  deleteNote,
} from "../utils/dexieAndSqliteDBServices";

export default function DexieAndSqlite({ isOnline }) {
  ...
  useEffect(() => {
    if (isOnline && !isLoading.initializing) {
      doSync();
    }
  }, [isOnline, isLoading.initializing]);
  ...
  if (isLoading.initializing) {
    return (
      <div className="m-2 bg-white/95 backdrop-blur-lg rounded-2xl p-6 shadow-lg border border-white/20">
    ...
    <div className="m-2 bg-white/95 backdrop-blur-lg rounded-2xl p-6 shadow-lg border border-white/20 transition-transform hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="bg-gray-100 rounded-md p-2 mb-4 text-sm">
        <div className="flex justify-between items-center">
          <span>
            📊 Total: {notes.length} | ✅ Synced: {syncedNotes.length} | ⚡
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 mb-4">
        <button
          onClick={addNote}
          disabled={isLoading.saving || !note.trim() || isLoading.initializing}
        ...
        <button
          ...
          disabled={isLoading.picking || isLoading.initializing}
          className={`px-4 py-2 rounded-lg font-semibold text-white ${
            isLoading.picking
        ...
        <button
          onClick={doSync}
          disabled={isLoading.syncing || isLoading.initializing}
            ...
            >
              <div className="flex-1 mb-2 sm:mb-0 sm:mr-4">
                {item.type === "note" ? (
                  ...
                  <div className="flex flex-col">
                    <img
                      src={item.blob ? URL.createObjectURL(item.blob) : ""}
                      alt="Offline"
              ...
              <div className="flex flex-col items-end gap-1">
                ...
                <button
                  ...
                  className="text-red-600 hover:bg-red-100 px-1 rounded text-xs"
                  title="Delete item"
                >
                  🗑️
                </button>
